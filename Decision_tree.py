import numpy as np
'''
Decision Tree implementation from scratch without using any libraries.
This implementation is for educational purposes and modifications may be made to improve its performance or accuracy.
.
'''
class DecisionTree:
    ...
    def __init__(self, max_depth=None):
        '''
        Initializes the Decision Tree with an optional maximum depth.
        Parameters:
        max_depth (int): The maximum depth of the tree. If None, the tree will
                         grow until all leaves are pure or until all features are used.
        '''
    ...
    def _predict_single(self, x, node):
        ''''      
            Predicts the label for a single input sample by traversing the tree.
            Parameters:
